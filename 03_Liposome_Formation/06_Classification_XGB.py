...
output_file_path = os.path.join(output_dir, 'XGB_incremental_features_evaluation.xlsx')
...
# Initialize lists for final model ROC
final_tprs = []
final_aucs = []
...
onehot_feature_names = column_transformer.transformers_[0][1].get_feature_names_out(['CHIP'])
other_feature_names = feature_columns[:2] + feature_columns[3:]  # Exclude the original 'CHIP' column
all_feature_names = np.concatenate([onehot_feature_names, other_feature_names])
all_feature_names = ['CHIP' if 'CHIP' in name else name for name in all_feature_names]

# Initialize the plot with adjusted size for final model ROC
fig, ax = plt.subplots(figsize=(7, 5))

for i, (train_idx, test_idx) in enumerate(cv_shap.split(X_scaled_final, y_binary)):
...
# Plot the mean ROC curve for the final model
mean_tpr_final = np.mean(final_tprs, axis=0)
mean_auc_final = np.mean(final_aucs)
std_auc_final = np.std(final_aucs)
ax.plot(np.linspace(0, 1, 100), mean_tpr_final, color=mean_color, label=f'Mean ROC (AUC = {mean_auc_final:.2f} ± {std_auc_final:.2f})', lw=3)  # Increased line width

# Plot the random guess line for the final model
ax.plot([0, 1], [0, 1], linestyle='--', lw=2, color='r', label='Random Guess')

# Configure plot for the final model ROC
ax.set_xlim([0.0, 1.0])
ax.set_ylim([0.0, 1.05])
ax.set_xlabel('False Positive Rate', fontsize=16, fontweight='bold')  # Bold label
ax.set_ylabel('True Positive Rate', fontsize=16, fontweight='bold')  # Bold label
ax.set_title('Receiver Operating Characteristic - Cross-Validation with All Features', fontsize=14, fontweight='bold')  # Bold title
ax.legend(loc='lower right', fontsize=11, prop={'weight':'bold'})  # Bold legend

# Increase size and boldness of tick labels
...
model_output_path = os.path.join(model_output_dir, 'xgb_Population_model.json')
...
# Save raw data for the ROC plot and SHAP plot to the Excel file
raw_data_roc = pd.DataFrame({'True Label': y_tests, 'Predicted Probability': y_probs, 'Predicted Label': y_preds})
raw_data_shap = pd.DataFrame(X_test_concat, columns=all_feature_names)
shap_values_df = pd.DataFrame(shap_values_concat, columns=[f'SHAP Value {i+1}' for i in range(shap_values_concat.shape[1])])

final_output_file_path = os.path.join(output_dir, 'XGB_incremental_features_evaluation.xlsx')
with pd.ExcelWriter(final_output_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
    raw_data_roc.to_excel(writer, sheet_name='ROC Raw Data', index=False)
    raw_data_shap.to_excel(writer, sheet_name='SHAP Raw Data', index=False)
