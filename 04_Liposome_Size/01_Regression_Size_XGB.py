...
# Filter out rows where 'Size' is greater than 100
#data = data[data['Size'] <= 100].reset_index(drop=True)
...
target_column = 'Size'  # Replace with your actual target column name
...
output_file_path = os.path.join(output_dir, 'XGB_incremental_features_evaluation_regression.xlsx')
...
other_feature_names = [name for name in feature_columns if name != 'CHIP']
all_feature_names = np.concatenate([onehot_feature_names, other_feature_names])

# Rename 'CHIP' features to 'Chip'
...
final_output_file_path = os.path.join(output_dir, 'XGB_final_evaluation_regression.xlsx')
...
model_output_path = os.path.join(model_output_dir, 'xgb_Size_model.json')
...
# Plot predicted size against actual size
...
plt.xlabel('Actual Size', fontsize=16, fontweight='bold')
plt.ylabel('Predicted Size', fontsize=16, fontweight='bold')
plt.title('Actual Size vs Predicted Size', fontsize=16, fontweight='bold')
plt.grid(True)
plt.tight_layout()
plt.show()
...
transformed_feature_names = np.concatenate([onehot_feature_names, [name for name in feature_columns if name != 'CHIP']])

# Convert the transformed data to a DataFrame
data_transformed_df = pd.DataFrame(data_transformed, columns=transformed_feature_names)

# Rename 'CHIP_ZZ' to 'Chip' in the heatmap
transformed_feature_names = ['Chip' if 'CHIP' in name else name for name in transformed_feature_names]
...
# clustering with Size using PCA and t-SNE
pca = PCA(n_components=5)
shap_pca50 = pca.fit_transform(shap_values_concat[:1000, :])

tsne = TSNE(n_components=2, perplexity=50)
shap_embedded = tsne.fit_transform(shap_values_concat[:1000, :])
...
cb = plt.colorbar(label="Predicted Size", aspect=40, orientation="horizontal")
cb.set_alpha(1)
cb.outline.set_linewidth(0)
cb.ax.tick_params("x", length=0)
cb.ax.xaxis.set_label_position("top")
plt.gca().axis("off")
plt.title("Supervised Clustering with Size", fontsize=16, fontweight='bold')
plt.show()
...
# xgb.plot_tree(model, num_trees=0, rankdir='LR', ax=plt.gca())
# plt.show()
...
# Plot the first tree in the model
# plt.figure(figsize=(20, 20))
# xgb.plot_tree(model, num_trees=99, rankdir='LR', ax=plt.gca())
# plt.show()

# Print the transformed feature names
# print("Transformed feature names:", all_feature_names)
...
plt.xlabel('Predicted Size (nm)', fontsize=16, fontweight='bold')
...
residuals_output_file_path = os.path.join(output_dir, 'XGB_final_evaluation_regression.xlsx')
